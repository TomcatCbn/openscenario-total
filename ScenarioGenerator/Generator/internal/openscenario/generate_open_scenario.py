...
def generate_xosc(frames, xodrFileName, file_path=GENERATE_FILE_DIR):
    ...
    return _createXoscWith(file_path, xodrFileName, 0.5, speedChangeEvents, actors, laneChangeEvents,
                           totalTime + 2), totalTime + 2


def _createXoscWith(file_path, xodrFileName, timeBegin, speedChangeEvents, actors, laneChangeEvents, totalTime=15):
    ...
    # create road
    dirs = xodrFileName.split('/')
    road = xosc.RoadNetwork(roadfile='./' + dirs[len(dirs) - 1], scenegraph='')
    ...
    file_absolute = file_path + os.path.basename(__file__).replace('.py', '.xosc')
    sce.write_xml(file_absolute)
