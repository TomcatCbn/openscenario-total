...
from .player.player import request_record2gif
...
def handle(need_gif=False):
    ...
    print('xodrFileName=%s\nxoscFileName = %s\n' % (xodrFileName, xoscFileName))
    # 仿真运行，移到对应esmini环境下
    if need_gif:
        # gif_file_path = record_gif(xodrFileName, xoscFileName, int(total_time))
        gif_file_path = request_record2gif(xodrFileName, xoscFileName, int(total_time))
    else:
        gif_file_path = ''
