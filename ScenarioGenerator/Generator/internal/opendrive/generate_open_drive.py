...
def generate_xodr(framesWrapLanes, file_path=GENERATE_FILE_DIR):
    if isinstance(framesWrapLanes, FramesWrapLanes):
        ...
        file_absolute = file_path + os.path.basename(__file__).replace('.py', '.xodr')
        odr.write_xml(file_absolute)
