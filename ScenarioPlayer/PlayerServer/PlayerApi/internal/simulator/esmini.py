...
def record_gif(xodr_file_name, xosc_file_name, length=10, resolution_x=320, resolution_y=240, frame_rate=10):
    if not os.path.exists(ESMINI_CMD_PATH):
        print('esmini is not support')
        return ''

    if not os.path.exists(ESMINI_SAMPLE_PATH):
        os.mkdir(ESMINI_SAMPLE_PATH)

    __exec_cmd('mv %s %s' % ('/root/temp/' + xodr_file_name, ESMINI_SAMPLE_PATH))
    __exec_cmd('mv %s %s' % ('/root/temp/' + xosc_file_name, ESMINI_SAMPLE_PATH))
    print('gif configs, length=%s, resolution=%sx%s, frame_rate=%s' % (
        str(length), str(resolution_x), str(resolution_y), str(frame_rate)))

    __exec_cmd(
        'timeout %d Xvfb :1 -screen 0 %dx%dx24 -nocursor &> xvfb.log &' % (length + 1, resolution_x, resolution_y))
    ...
    gif_save_path = '/root/temp/' + gif_name
    __exec_cmd(
